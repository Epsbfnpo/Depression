...
class Mamba(nn.Module):
    ...
    def forward(self, a_hidden_states, v_hidden_states, a_inference_params=None, v_inference_params=None):
        ...
        if self.use_fast_path and a_inference_params is None and v_inference_params is None:
            ...
            if a_conv_state is not None:
                a_conv_state.copy_(F.pad(a_x, (self.d_conv - a_x.shape[-1], 0)))
            if v_conv_state is not None:
                v_conv_state.copy_(F.pad(v_x, (self.d_conv - v_x.shape[-1], 0)))
            if causal_conv1d_fn is None:
                a_x = self.act(self.a_conv1d(a_x)[..., :seqlen])
                v_x = self.act(self.v_conv1d(v_x)[..., :seqlen])
            else:
                assert self.activation in ["silu", "swish"]
                a_x = causal_conv1d_fn(x=a_x, weight=rearrange(self.a_conv1d.weight, "d 1 w -> d w"), bias=self.a_conv1d.bias, activation=self.activation,)
                v_x = causal_conv1d_fn(x=v_x, weight=rearrange(self.v_conv1d.weight, "d 1 w -> d w"), bias=self.v_conv1d.bias, activation=self.activation,)
            a_x_dbl = self.a_x_proj(rearrange(a_x, "b d l -> (b l) d"))
            ...
            v_x_dbl = self.v_x_proj(rearrange(v_x, "b d l -> (b l) d"))
    ...
    def step(self, a_hidden_states, a_conv_state, a_ssm_state, v_hidden_states, v_conv_state, v_ssm_state):
        ...
        a_xz = self.a_in_proj(a_hidden_states.squeeze(1))
        a_x, a_z = a_xz.chunk(2, dim=-1)
        v_xz = self.v_in_proj(v_hidden_states.squeeze(1))
        v_x, v_z = v_xz.chunk(2, dim=-1)
        if causal_conv1d_update is None:
            a_conv_state.copy_(torch.roll(a_conv_state, shifts=-1, dims=-1))
            a_conv_state[:, :, -1] = a_x
            a_x = torch.sum(a_conv_state * rearrange(self.a_conv1d.weight, "d 1 w -> d w"), dim=-1)
            if self.a_conv1d.bias is not None:
                a_x = a_x + self.conv1d.bias
            a_x = self.act(a_x).to(dtype=dtype)
            v_conv_state.copy_(torch.roll(v_conv_state, shifts=-1, dims=-1))
            v_conv_state[:, :, -1] = v_x
            v_x = torch.sum(v_conv_state * rearrange(self.v_conv1d.weight, "d 1 w -> d w"), dim=-1)
            if self.v_conv1d.bias is not None:
                v_x = v_x + self.conv1d.bias
            v_x = self.act(v_x).to(dtype=dtype)
        else:
            a_x = causal_conv1d_update(a_x, a_conv_state, rearrange(self.a_conv1d.weight, "d 1 w -> d w"), self.a_conv1d.bias, self.activation,)
            v_x = causal_conv1d_update(v_x, v_conv_state, rearrange(self.v_conv1d.weight, "d 1 w -> d w"), self.v_conv1d.bias, self.activation,)
        a_x_db = self.a_x_proj(a_x)
        a_dt, a_B, a_C = torch.split(a_x_db, [self.dt_rank, self.d_state, self.d_state], dim=-1)
        a_dt = F.linear(a_dt, self.a_dt_proj.weight)
    ...
    def allocate_inference_cache(self, batch_size, max_seqlen, dtype=None, **kwargs):
        ...
        a_conv_state = torch.zeros(batch_size, self.d_model * self.expand, self.d_conv, device=device, dtype=conv_dtype)
        a_ssm_state = torch.zeros(batch_size, self.d_model * self.expand, self.d_state, device=device, dtype=ssm_dtype)
    ...
    def a_get_states_from_cache(self, inference_params, batch_size, initialize_states=False):
        ...
        if self.layer_idx not in inference_params.key_value_memory_dict:
            ...
            conv_state = torch.zeros(batch_size, self.d_model * self.expand, self.d_conv, device=self.a_conv1d.weight.device, dtype=self.a_conv1d.weight.dtype,)
            ssm_state = torch.zeros(batch_size, self.d_model * self.expand, self.d_state, device=self.a_dt_proj.weight.device, dtype=self.a_dt_proj.weight.dtype,)
    ...
    def v_get_states_from_cache(self, inference_params, batch_size, initialize_states=False):
        ...
        if self.layer_idx not in inference_params.key_value_memory_dict:
            batch_shape = (batch_size,)
            conv_state = torch.zeros(batch_size, self.d_model * self.expand, self.d_conv, device=self.v_conv1d.weight.device, dtype=self.v_conv1d.weight.dtype,)
            ssm_state = torch.zeros(batch_size, self.d_model * self.expand, self.d_state, device=self.v_dt_proj.weight.device, dtype=self.v_dt_proj.weight.dtype,)
            inference_params.key_value_memory_dict[self.layer_idx] = (conv_state, ssm_state)
        else:
            conv_state, ssm_state = inference_params.key_value_memory_dict[self.layer_idx]
            # TODO: What if batch size changes between generation, and we reuse the same states?
