...
def get_lmvd_dataloader(root: Union[str, Path], fold: str="train", batch_size: int=8, gender: str="both", transform=None, target_transform=None, aug=True):
...
if __name__ == '__main__':
    train_loader = get_lmvd_dataloader(root="./dataset/lmvd", fold="train")
    print(f"train_loader: {len(train_loader.dataset)} samples")
    valid_loader = get_lmvd_dataloader(root="./dataset/lmvd", fold="valid")
    print(f"valid_loader: {len(valid_loader.dataset)} samples")
    test_loader = get_lmvd_dataloader(root="./dataset/lmvd", fold="test")
